...
use Elibrary\Lms\Payments\PaystackGateway;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\Http;
use Tests\TestCase;

class LmsCoursePaystackPaymentTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function paidCourse(Tenant $tenant): Course
    {
        return Course::create([
            ...
            'pricing_policy' => 'paid', 'price' => 10, 'currency' => 'USD',
        ]);
    }

    private function enablePaystack(User $owner, string $secretKey = 'sk_test_paystack'): void
    {
        $this->actingAs($owner)->put('/t/acme/lms/manage/payment-gateways/paystack', [
            'is_enabled' => '1',
    ...
    {
        $gateway = app(PaymentGatewayFactory::class)->make(CoursePaymentGateway::Paystack);
    ...
    public function test_choosing_paystack_redirects_the_learner_to_the_authorization_url(): void
    {
        ...
        $response = $this->actingAs($user)->post('/t/acme/lms/courses/algebra/purchase', ['gateway' => 'paystack']);
    ...
    public function test_a_valid_signature_webhook_marks_the_purchase_paid_and_enrolls(): void
    {
        [$tenant, $owner] = $this->tenantWithOwner();
        ...
        $this->actingAs($user)->post('/t/acme/lms/courses/algebra/purchase', ['gateway' => 'paystack']);
        $purchase = CoursePurchase::where('course_id', $course->id)->first();
        ...
        $response = $this->call('POST', '/webhooks/courses/paystack', [], [], [], [
    ...
    public function test_an_invalid_signature_is_rejected_and_does_not_enroll(): void
    {
        [$tenant, $owner] = $this->tenantWithOwner();
        $this->enablePaystack($owner, 'sk_test_secret');
        $user = User::factory()->create(['tenant_id' => $tenant->id]);
