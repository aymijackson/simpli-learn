...
use Elibrary\Lms\Payments\PaymentGatewayContract;
use Elibrary\Lms\Payments\PaymentGatewayFactory;
use Elibrary\Lms\Payments\StripeGateway;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Http\Request;
use Illuminate\Support\Str;
use Tests\TestCase;

class LmsCourseStripePaymentTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $gateway = app(PaymentGatewayFactory::class)->make(CoursePaymentGateway::Stripe);
    ...
    public function test_a_valid_signature_is_accepted_and_an_invalid_one_is_rejected(): void
    {
        ...
        $credential = CoursePaymentGatewayCredential::create([
            'tenant_id' => $tenant->id,
            'gateway' => 'stripe',
            'is_enabled' => true,
        ...
        $validRequest = Request::create('/webhooks/courses/stripe', 'POST', [], [], [], [], $payload);
        ...
        $invalidRequest = Request::create('/webhooks/courses/stripe', 'POST', [], [], [], [], $payload);
    ...
    public function test_resolve_purchase_from_webhook_finds_the_matching_purchase(): void
    {
        [$tenant] = $this->tenantWithOwner();
        ...
        $purchase = CoursePurchase::create([
            ...
            'purchase_type' => 'enrollment', 'gateway' => 'stripe', 'amount' => 10, 'currency' => 'USD',
        ...
        $request = Request::create('/webhooks/courses/stripe', 'POST', [], [], [], [], $payload);

        $resolved = (new StripeGateway)->resolvePurchaseFromWebhook($request);
    ...
    public function test_the_webhook_endpoint_rejects_an_invalid_signature_and_does_not_mark_the_purchase_paid(): void
    {
        ...
        CoursePaymentGatewayCredential::create([
        ...
        $response = $this->call('POST', '/webhooks/courses/stripe', [], [], [], [
    ...
    {
        $this->post('/webhooks/courses/not-a-real-gateway')->assertNotFound();
    }
}
