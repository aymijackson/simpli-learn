<x-app-layout title="Get Certificate">
    ...
    <x-page-header title="Get your certificate" :subtitle="$exam->title" />

    <x-card class="mb-6 text-center">
        <p class="text-3xl font-bold text-slate-900">{{ number_format($price, 2) }} {{ $currency }}</p>
        <p class="mt-1 text-sm text-slate-500">One-time payment for a verified certificate.</p>
    </x-card>
    ...
    @else
        <x-card>
            <form method="POST" action="{{ route('cbt.attempts.certificate-payment.store', $attempt) }}" class="space-y-4">
                @csrf
