<x-app-layout title="Resource Payment Gateways">
    <x-page-header title="Resource payment gateways" subtitle="Configure how members pay for paid resources." />

    <div class="space-y-6">
        @foreach ($gateways as $gateway)
            @php($credential = $credentials[$gateway->value])
            <x-card>
                <form method="POST" action="{{ route('library.manage.payment-gateways.update', $gateway->value) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    @else
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-slate-700">Bank account details (shown to members)</label>
